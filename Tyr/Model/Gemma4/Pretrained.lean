...
private def hasGemma4WeightFiles (dir : String) : IO Bool := do
...
def findCachedSnapshot? (repoId : String) (revision : String := "main") : IO (Option String) := do
  ...
  for c in candidates do
    let cStr := c.toString
    if (← fileExists s!"{cStr}/config.json") && (← hasGemma4WeightFiles cStr) then
      return some cStr
  pure none
...
private def ensureTokenizerFiles (repoId revision modelDir : String) : IO Unit := do
  ...
  ensureRemoteFile repoId revision "processor_config.json" s!"{modelDir}/processor_config.json"
  pure ()
