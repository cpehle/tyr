...
/-! ## ShARK Stochastic Runge-Kutta Solver (additive noise, Stratonovich) -/

structure ShARK where
  deriving Inhabited

def ShARK.solver {Drift Diffusion Y VFg Control Args : Type}
  ...
  step := fun terms t0 t1 y0 args state _madeJump =>
    ...
    let z0 := DiffEqSpace.add y0 h_kg
    let h_kf0 := driftInst.vf_prod drift t0 z0 args dt
    let drift1 := DiffEqSpace.scale (5.0 / 6.0) h_kf0
    let diff1 := DiffEqSpace.add (DiffEqSpace.scale (5.0 / 6.0) w_kg) h_kg
    let z1 := DiffEqSpace.add y0 (DiffEqSpace.add drift1 diff1)
    let h_kf1 := driftInst.vf_prod drift (t0 + (5.0 / 6.0) * dt) z1 args dt
    let drift_result :=
      DiffEqSpace.add (DiffEqSpace.scale 0.4 h_kf0) (DiffEqSpace.scale 0.6 h_kf1)
...
instance : ExplicitSolver ShARK := ⟨True.intro⟩
instance : StratonovichSolver ShARK := ⟨True.intro⟩
