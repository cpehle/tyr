...
private def b200Fp8Accumulator {inDtype : GpuFloat}
    (banner : String)
    (sourceNote : String)
    (aPtr : GPtr inDtype)
    (bPtr : GPtr inDtype)
...
def tkB200Fp8E4M3Gemm1CtaFwd
    ...
    (k : KVal UInt64)
    : KernelM Unit := do
  let (accum, coord) ← b200Fp8Accumulator
    "=== B200 FP8 GEMM (1 CTA) ==="
    "ThunderKittens fp8_b200_gemm_1cta reduced to a typed 128x256 Blackwell-sized CTA mainloop"
    aPtr bPtr m n k
...
def tkB200Fp8E4M3Gemm2CtaFwd
    ...
    : KernelM Unit := do
  let (accum, coord) ← b200Fp8Accumulator
    "=== B200 FP8 GEMM (2 CTA cluster) ==="
    "ThunderKittens fp8_b200_gemm_2cta represented as the same typed Blackwell tile family with cluster-sized output geometry"
    aPtr bPtr m n k
  GemmCommon.storeConvertedTile dPtr coord accum
...
def tkB200MxFp8GemmFwd
  ...
  let (accum, coord) ← b200Fp8Accumulator
    "=== B200 MXFP8 GEMM ==="
    "ThunderKittens mxfp8_b200 expressed as a typed Blackwell FP8 mainloop plus explicit e8m0 row/column scale epilogue"
    aPtr bPtr m n k
  let (scaleA, scaleB) ← GemmCommon.loadRowColScaleVectorsAsFloat32
    (tileM := b200TileM)
    (tileN := b200TileN)
    scaleAPtr scaleBPtr coord
  let scaled ← GemmCommon.applyRowColScales accum scaleA scaleB
