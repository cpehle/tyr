...
private def mhaH100LcfFwd
  ...
  let q : RT GpuFloat.BFloat16 qRows headDim ← allocRT .BFloat16 qRows headDim
  ...
  let o : RT GpuFloat.Float32 qRows headDim ← zeroRT .Float32 qRows headDim

  let softmaxState ← allocSoftmaxState .Float32 qRows

  let qShared : ST GpuFloat.BFloat16 qRows headDim ← allocST .BFloat16 qRows headDim
  ...
  let oShared : ST GpuFloat.BFloat16 qRows headDim ← allocST .BFloat16 qRows headDim

  comment banner
  comment "Stationary 64xD query tile with streamed larger-KV online-softmax updates"
  comment "Worker packing from the CUDA lcf template is represented as one logical query worker per kernel instance"
  ...
  for kvIdx in krange 0 numKvTiles do
    let scores : RT GpuFloat.Float32 qRows kvTileRows ← zeroRT .Float32 qRows kvTileRows
    let probs : RT GpuFloat.BFloat16 qRows kvTileRows ← allocRT .BFloat16 qRows kvTileRows
  ...
  let out : RT GpuFloat.BFloat16 qRows headDim ← allocRT .BFloat16 qRows headDim
  convert out o
  store oShared out
