...
/-!
# Tyr.GPU.Codegen.Stmt

The statement vocabulary for lowered GPU kernels.

This module intentionally sits below `IR` and `Proof`: both kernel containers
and proof certificates need to refer to real statements without creating an
import cycle.
-/
...
inductive KStmt where
  ...
  | declTMEMPool (v : VarId) (slots clusterSize : Nat) (managed : Bool) -- tensor_allocator<>
  ...
  | atomicFetchAddUInt32 (dst ptr increment : VarId) -- dst <- atomicAdd(ptr, increment), ptr is uint32_t*
  | warpBroadcast (dst src : VarId) (srcLane : Nat) (mask : Nat)
