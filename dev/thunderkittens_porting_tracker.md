...
| `attention/mha_h100_lcf/mha_h100_lcf.cu` | `Tyr/GPU/Kernels/MhaH100LCF.lean` (`tkMhaH100LCFFwd64`, `tkMhaH100LCFFwd128`) | `partial` | Dedicated LCF forward surfaces exist; CTA multi-worker packing is compressed to one logical query worker per kernel instance. |
...
| `gemm/bf16_b200/bf16_b200_gemm.cu` | `Tyr/GPU/Kernels/Bf16Gemm.lean` (`tkB200Bf16GemmCompatFwd`) | `compatibility` | Dedicated Blackwell BF16 counterpart exists, but exact TMEM/cluster overlap remains outside the current DSL. |
...
| `gemm/fp8_b200/fp8_b200_gemm_1cta.cu` | `Tyr/GPU/Kernels/PrecisionGemm.lean` (`tkB200Fp8E4M3Gemm1CtaCompatFwd`) | `compatibility` | Dedicated 1-CTA Blackwell FP8 counterpart exists; TMEM-backed output staging is flattened to CTA-local storage. |
| `gemm/fp8_b200/fp8_b200_gemm_2cta.cu` | `Tyr/GPU/Kernels/PrecisionGemm.lean` (`tkB200Fp8E4M3Gemm2CtaCompatFwd`) | `compatibility` | Dedicated 2-CTA Blackwell FP8 counterpart exists; cluster exchange remains compressed. |
...
| `gemm/mxfp8_b200/mxfp8_b200_gemm.cu` | `Tyr/GPU/Kernels/PrecisionGemm.lean` (`tkB200MxFp8GemmCompatFwd`) | `compatibility` | Dedicated MXFP8 counterpart exists with Float32 scale proxies standing in for packed `e8m0` scale tiles. |
| `gemm/nvfp4_b200/nvfp4_b200_gemm.cu` | `Tyr/GPU/Kernels/NvFp4Gemm.lean` (`tkB200NvFp4GemmCompatFwd`) | `compatibility` | Explicit control-flow port, limited by missing packed NVFP4 types. |
...
| `Tyr/GPU/Kernels/MhaH100LCF.lean` | `kernels/attention/mha_h100_lcf/mha_h100_lcf.cu` | Dedicated LCF forward surfaces now exist for the two benchmark head dimensions; CTA worker packing is still compressed. |
...
| `Tyr/GPU/Kernels/Bf16Gemm.lean` | `kernels/gemm/bf16_h100/bf16_h100_gemm.cu`, `kernels/gemm/bf16_b200/bf16_b200_gemm.cu` | H100 BF16 source counterpart is now dedicated; the B200 side is present as a compatibility surface until TMEM/cluster scheduling are modeled directly. |
...
| `Tyr/GPU/Kernels/Bf16Gemm.lean` | `kernels/gemm/bf16_*.cu` | H100 BF16 is source-backed; B200 BF16 still compresses TMEM/cluster scheduling into a CTA-local compatibility surface. |
| `Tyr/GPU/Kernels/PrecisionGemm.lean` | `kernels/gemm/fp8_h100/*`, `kernels/gemm/fp8_b200/*`, `kernels/gemm/mxfp8_b200/*` | Dedicated H100 and B200/MXFP8 surfaces now exist; the remaining gap is exact Blackwell TMEM / packed-scale modeling rather than missing source families. |
