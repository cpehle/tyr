...
- [~] `DX06` [Medium] Expanded Brownian support beyond scalar `Float` with shape-aware sampling (`Fin n → BM`, `Vector n BM`), pair-valued `UnsafeBrownianPath`/`VirtualBrownianTree` conveniences, pair `VirtualBrownianTreeOps` dt normalization, structured space-time Lévy regressions, broader structured-state arithmetic instances (`(Y1 × Y2)`, `Fin n → Y`, `Vector/List/Array/Option`), container `VirtualBrownianTreeOps` parity for `Array BM`/`List BM` with additivity regressions, `Fin`-container virtual-tree materialization parity improvements, and `Option BM` virtual-tree container semantics/tests (`none` propagation, `some` additivity/consistency).
