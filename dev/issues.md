...
- [~] `DX01` [High] Event parity advanced with real-condition directionality (`up`/`down`/either), multi-event solve args, per-event masks, and tie-breaking behavior (terminating-on-tie preference) with regression coverage; richer event-tree semantics still remain.
...
- [~] `DX03` [High] Added integer `steps=n`, nested `SubSaveAt` aggregation, custom save transform support (`saveFn`), and solve-direction monotonic validation for `saveat.ts`; full diffrax-style `SubSaveAt` payload tree parity is still pending.
