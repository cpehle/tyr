...
- [~] `DX03` [High] Added integer `steps=n`, nested `SubSaveAt` aggregation, custom save transform support (`saveFn`), solve-direction monotonic validation for `saveat.ts`, deterministic nested payload traversal/output tests (including mixed `ts+steps`), and diffrax-style leaf-only `SubSaveAt` payload semantics (non-leaf nodes now act as structural containers).
...
- [~] `DX05` [High] Added adjoint-mode dispatch APIs with explicit unsupported-matrix reporting, recursive-checkpoint recomputation/backprop, forward-mode inferred-`dt0` support (when explicitly allowed), improved implicit recursive-fallback diagnostics/tests, and implicit-adjoint save-config contract parity (`saveat=SaveAt(t1=True)` only) with explicit unsupported-tag diagnostics.
  Refs: `Tyr/DiffEq/Adjoint/Core.lean`, `Tests/TestDiffEqAdjointParity.lean`, `../diffrax/diffrax/_adjoint.py`
- [~] `DX06` [Medium] Expanded Brownian support beyond scalar `Float` with shape-aware sampling (`Fin n → BM`, `Vector n BM`), pair-valued `UnsafeBrownianPath`/`VirtualBrownianTree` conveniences, pair `VirtualBrownianTreeOps` dt normalization, structured space-time Lévy regressions, broader structured-state arithmetic instances (`(Y1 × Y2)`, `Fin n → Y`, `Vector/List/Array/Option`), and container `VirtualBrownianTreeOps` parity for `Array BM`/`List BM` with additivity regressions.
  Refs: `Tyr/DiffEq/Brownian.lean`, `Tyr/DiffEq/Types.lean`, `Tests/TestDiffEq.lean`, `Tests/TestDiffEqContainerBrownian.lean`, `../diffrax/diffrax/_brownian/path.py`, `../diffrax/diffrax/_brownian/tree.py`, `../diffrax/diffrax/_custom_types.py`
...
- [~] `DX08` [Medium] Milstein now supports control-aware Jacobian-product interfaces, user-injected autodiff/JVP-style callbacks, stronger finite-difference fallbacks for non-scalar controls, Stratonovich-Milstein JVP-wrapper regressions, and vector-control finite-diff wrapper parity checks; full autodiff parity remains pending.
...
- [~] `DX11` [Low] Added path-derivative support, `ControlTerm.to_ode` conversions, reusable path constructors (linear/cubic Hermite interpolation), `AbstractPath.compose`/`restrict`/`mapControl` regression coverage, and diffrax-style `evaluate(t, None)` point-value semantics (while retaining increment semantics for `evaluate(t0, Some t1)`).
  Refs: `Tyr/DiffEq/Path.lean`, `Tyr/DiffEq/Term.lean`, `Tyr/DiffEq/Interpolation.lean`, `Tests/TestDiffEqPathParity.lean`, `../diffrax/diffrax/_path.py`, `../diffrax/diffrax/_global_interpolation.py`, `../diffrax/diffrax/_term.py`
...
- [~] `DX14` [Medium] Added convergence/order regressions (Euler first-order trend, RK4 high-order trend, dense interpolation error trends including Kvaerno3/KenCarp3 implicit-IMEX checks, Dopri5/Tsit5 dense polynomial-vs-Hermite comparisons, EM strong-order trend, and Milstein-vs-EM pathwise advantage over seeded Brownian ensembles); full solver-by-solver order certification and broader statistical coverage are still pending.
  Refs: `Tests/TestDiffEq.lean`, `Tyr/DiffEq/Solver/*.lean`, `../diffrax/diffrax/_solver/*`
- [~] `DX15` [Low] Added deterministic weak-order Monte-Carlo regression coverage for Euler-Maruyama (OU mean/second-moment convergence across step refinements) and higher-moment weak regressions (third/fourth moments with step-halving trend checks) guided by diffrax SDE order docs; broader weak-statistical parity across SDE families still remains.
  Refs: `Tests/TestDiffEq.lean`, `Tests/TestDiffEqWeakStats.lean`, `Tyr/DiffEq/Solver/SDE.lean`, `../diffrax/docs/devdocs/SDE_solver_table.md`
