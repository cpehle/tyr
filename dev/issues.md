...
- [~] `DX19` [Medium] Added `progress_meter` solve API surface (`none`/`text`/`tqdm`) with lifecycle hooks and compatibility aliasing (`tqdm` -> lightweight text behavior), plus regression coverage for default/text/tqdm parity including event-termination and `saveat.steps` invariants, and deterministic rendered-cadence parity checks for progress-throttled text/tqdm updates with forced terminal close emission on early termination.
