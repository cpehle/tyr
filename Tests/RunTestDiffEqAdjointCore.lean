...
private def parseArgs (args : List String) : IO LeanTest.RunConfig := do
  ...
  while _h : !remaining.isEmpty do
    ...
    | "--fail-fast" :: rest =>
      ...
      remaining := rest
    | "--help" :: _ =>
      IO.println "Usage: TestDiffEqAdjointCore [OPTIONS]"
      IO.println ""
      IO.println "Options:"
...
/-- Standalone test runner for the core DiffEq adjoint test module. -/
unsafe def main (args : List String) : IO UInt32 := do
  let config ← parseArgs args
  Lean.initSearchPath (← Lean.findSysroot)
  Lean.enableInitializersExecution
  let env ← Lean.importModules
    #[{ module := `LeanTest }, { module := `Tests.TestDiffEqAdjointCore }]
    {}
