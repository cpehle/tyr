...
private def parseArgs (args : List String) : IO LeanTest.RunConfig := do
  ...
  while _h : !remaining.isEmpty do
    ...
    | "--fail-fast" :: rest =>
      ...
      remaining := rest
    | "--help" :: _ =>
      IO.println "Usage: RunTestADNumerics [OPTIONS]"
      IO.println ""
      IO.println "Options:"
...
/-- Standalone test runner for numeric AD/elimination coverage. -/
unsafe def main (args : List String) : IO UInt32 := do
  ...
  let env ← Lean.importModules
    #[
      { module := `LeanTest },
      { module := `Tests.TestADElimNumerics },
      { module := `Tests.TestADSparse }
    ]
    {}
